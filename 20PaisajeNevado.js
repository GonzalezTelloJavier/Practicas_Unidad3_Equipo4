      ...
      function dibujaCopo(num, x, y){
        var tama = Math.round(Math.random() * 10) + 10;
    ...
    for (i = 0; i < window.copos.length; i++) {
        ...
        window.copos[i].style.top = y + 'px';
        if (y > document.body.scrollTop + window.coposAlto) {
            window.copos[i].style.top = document.body.scrollTop -
                Math.round(Math.random() * 10) + 'px';
            window.copos[i].style.left =
                parseInt(Math.random() * window.coposAncho - 1) + 'px';
        }
    }
    setTimeout(nevar, window.coposVeloc);
}
